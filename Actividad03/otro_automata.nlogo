...
to setup
  clear-all
  ask patches [
    ifelse random-float 1.0 < prop_celulas [
      set estado 1   ;; Estado vivo
      set tiempo_muerta 0
    ][
      set estado 0   ;; Estado muerto
...
to go
  ;; Primero, contar cuántos vecinos vivos tiene cada célula
  ask patches [
...
to colorear_celdas
  ask patches [
    if estado = 1 [ set pcolor white ]   ;; Células vivas son blancas
    if estado = 2 [ set pcolor red ]     ;; Células moribundas son rojas
    if estado = 0 [
