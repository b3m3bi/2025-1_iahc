...
depredadores-own [ genotipo energia ]
...
to crear-alimento
  ...
  create-alimentos num-alimento [
    ...
    set size 0.5
    set color white
    ifelse alimento-aleatorio?
    [ ; Coloca los alimentos en posiciones aleatorias
        setxy random-xcor random-ycor
      ][ ; Coloca los alimentos en posiciones específicas
        setxy (world-width / 4) (world-height / 4)
      ]
  ]
end
...
to reproducirse
  ...
  while [ count tortugas with [ energia = 0 ] < poblacion ] [
    let padre1 one-of tortugas with [ energia != 0 ]
    let padre2 one-of tortugas with [ energia != 0 and self != padre1 ]
    if padre1 != padre2 [
      ask padre1 [
        if random-float 1.0 < (energia / max-rep) [
          let genotipo-padre1 genotipo
          ask padre2 [
            let genotipo-padre2 genotipo
            hatch-tortugas 1 [
              set energia 0
              let mezcla []
              ; Mezclar genotipos de ambos padres
              repeat num-genes [
                let indice-gen random num-genes
                let gene (ifelse-value (random 2 = 0)
                  [ item indice-gen genotipo-padre1 ]
                  [ item indice-gen genotipo-padre2 ])
                set mezcla lput gene mezcla
              ]
              set genotipo mezcla
              posicionar-tortuga
...
En lugar de que los hijos hereden su genotipo de un solo progenitor, el genotipo del hijo es una combinación de dos tortugas. Seleccionando dos tortugas al azar, mezclamos partes de sus genotipos para crear un nuevo individuo.

## Evolución de depredadores con redes neuronales

Aplicamos la misma lógica de redes neuronales a los depredadores que ya tienen las tortugas. Agregación de genotipo y energia a depredadores-own, y modificación a la función moverse-dep para que los depredadores usen su red neuronal al tomar decisiones de movimiento.

## Modificación de la posición de la fuente de alimento

Implementacion de un control de posición de alimentos para modificar la creacion de alimentos y los posicione de acuerdo a la selección:
